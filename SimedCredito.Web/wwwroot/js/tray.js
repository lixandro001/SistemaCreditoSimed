...
function fnLoadTrayUser() {
  ...
  //  var documentNumber = "48296633";
    ...
    Get("Receipt/GetReceiptUser?startdate=" + startDate + "&enddate=" + endDate).done(function (response) {

        console.log(response);
        if (response.data != null) {
            fnClearTable($('#tabTray').dataTable());
            if (response.data.length > 0) {
                $('#tabTray').dataTable().fnAddData(response.data);
                eventDetail();
                poppupSaveMail();
            } else {
